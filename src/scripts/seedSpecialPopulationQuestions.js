...
const seedSpecialPopulationQuestions = async () => {
  try {
    await connectDB();
    
    // Add questions for special populations
    ...
    await QuestionSuggestion.insertMany(specialPopulationQuestions);
    
    console.log('Special population question suggestions seeded successfully');
    process.exit(0);
  } catch (error) {
    console.error('Error seeding special population question suggestions:', error);
    process.exit(1);
  }
};

seedSpecialPopulationQuestions();