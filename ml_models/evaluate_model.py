# -*- coding: utf-8 -*-
"""
Script đánh giá độ chính xác của mô hình Swin + KNN trên tập dữ liệu thuốc
...
Output:
- Báo cáo độ chính xác tổng thể
- Báo cáo chi tiết theo từng loại thuốc
"""
import os
import sys
import time
...
KNN_MODEL_PATH = os.path.join(BASE_DIR, "knn_model_swin_b_k5_cosine.joblib")
...
# Thêm hàm để tải thông tin ánh xạ từ folder number sang class ID
...
def main():
    # Parse arguments
    parser = argparse.ArgumentParser(description='Đánh giá độ chính xác của mô hình Swin + KNN')
    parser.add_argument('--data-dir', type=str, required=True,
                        help='Đường dẫn đến thư mục dữ liệu theo cấu trúc: thư_mục/id_thuốc/ảnh')
    parser.add_argument('--swin-model', type=str, default=SWIN_MODEL_PATH,
                        help='Đường dẫn đến file model Swin Transformer đã train')
    parser.add_argument('--knn-model', type=str, default=KNN_MODEL_PATH,
                        help='Đường dẫn đến file model KNN')
    parser.add_argument('--class-map', type=str, default=CLASS_MAP_PATH,
                        help='Đường dẫn đến file class map')
    parser.add_argument('--folder-map', type=str, default=None,
                        help='Đường dẫn đến file ánh xạ thư mục sang class ID (JSON)')
    ...
    # Tải class mapping
    class_to_idx, idx_to_class, num_classes = load_class_mapping(args.class_map)
    if idx_to_class is None:
        logger.error("Không thể tải class mapping. Dừng đánh giá.")
        return
    
    # Tải folder-to-class mapping nếu có
    folder_to_class = None
    if args.folder_map:
        folder_to_class = load_folder_to_class_mapping(args.folder_map)
    
    # Tải Swin feature extractor
    feature_extractor = get_swin_feature_extractor("swin_b", args.swin_model, num_classes, device)
    if feature_extractor is None:
        logger.error("Không thể tải Swin feature extractor. Dừng đánh giá.")
        return
    
    # Tải KNN model
    logger.info(f"Đang tải KNN model từ {args.knn_model}")
    try:
    ...
    # Bắt đầu đánh giá
    logger.info(f"Bắt đầu đánh giá trên tập dữ liệu: {args.data_dir}")
    start_time = time.time()
    
    results = evaluate_dataset(args.data_dir, feature_extractor, transform, knn_model, idx_to_class, folder_to_class)
    if results is None:
    ...
    class_stats_path = os.path.join(args.output_dir, 'class_stats.csv')
    class_stats_df.to_csv(class_stats_path, index=False)
    logger.info(f"Đã lưu thống kê theo class vào {class_stats_path}")
    
    logger.info(f"Tất cả kết quả đã được lưu vào thư mục: {args.output_dir}")
